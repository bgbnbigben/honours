function language = nomadm_compile(fileType,fileName,fileExt)
...
switch upper(fileType)
   case 'F'
      compileFortran(fileName,fileExt);
      language = 'FORTRAN';
   case 'C'
      compileC(fileName,fileExt);
      language = 'C';
   case 'M'
      warning('nomadm_compile:FileType', ...
              'Matlab Function file found.  No compiling is necessary.');
      language = 'Matlab';
   otherwise
      error('NOMADmCompile:FileType','Cannot compile: Incompatible File Type');
end
return
...
%*******************************************************************************
function compileFortran(fileName,fortranExt)

% Construct the names of the gateway file and compiled file
userFile    = [fileName,fortranExt];
wrapperFile = [fileName,'g',fortranExt];
execFile    = [fileName,'.',mexext];
...
if (~exist(execFile,'file'))
   mex(userFile,wrapperFile);
end
return

%*******************************************************************************
% compileC:  Construct a MEX C gateway file.
% ------------------------------------------------------------------------------
% Called by:  nomadm_compile
% VARIABLES:
%  fileName    = optimization problem name
...
%  fid         = file ID -- handle for the file that is being written
%*******************************************************************************
function compileC(fileName,cExt)

% Construct the names of the gateway file and compiled file
userFile    = [fileName,cExt];
wrapperFile = [fileName,'temp',cExt];
execFile    = [fileName,'.',mexext];
...
if (~exist(execFile,'file'))
   ...
   mex(wrapperFile,'-output', fileName);
