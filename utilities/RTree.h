...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, 
    ...
    class RTree {
        ...
        public:
            ...
            enum {
                MAXNODES = TMAXNODES,                         ///< std::max elements in node
                MINNODES = TMINNODES,                         ///< std::min elements in node
            };
        ...
        public:
            ...
            /// Insert entry
            /// \param a_min std::min of bounding rect
            /// \param a_max std::max of bounding rect
            ...
            /// \param a_min std::min of search bounding rect
            /// \param a_max std::max of search bounding rect
            /// \param a_searchResult Search result array.  Caller should set grow size. Function will reset, not append to array.
            /// \param a_resultCallback Callback function to return result.  Callback should return 'true' to continue searching
            /// \param a_context User context to pass as parameter to a_resultCallback
            /// \return Returns the number of entries found
            unsigned Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context);
            ...
            class Iterator {
                private:
                    ...
                    enum { MAX_STACK = 32 }; //  std::max stack size. Allows almost n^32 where n is number of branches in node
                    struct StackElement {
                        Node* m_node;
                        int m_branchIndex;
                    };
        ...
        protected:
            ...
            struct Rect {
                ELEMTYPE m_min[NUMDIMS];                      ///< std::min dimensions of bounding box 
                ELEMTYPE m_max[NUMDIMS];                      ///< std::max dimensions of bounding box 
            };
            ...
            bool Search(Node* a_node, Rect* a_rect, unsigned& a_foundCount, t_resultCallback a_resultCallback, void* a_context);
...
RTREE_QUAL::RTree() {
    ...
    ELEMTYPEREAL a = 1.0;
    ELEMTYPEREAL b = 2.0;
    if (NUMDIMS == 0) m_unitSphereVolume = a;  
    else if (NUMDIMS == 1) m_unitSphereVolume = b;  
    else {
        for (int i = 2; i <= NUMDIMS; i++) {
            double c = 2.0 * (ELEMTYPEREAL)PI / (ELEMTYPEREAL) i * a;
            a = b; 
            b = c; 
            m_unitSphereVolume = c;
        }
    }
}
...
RTREE_TEMPLATE
unsigned RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context) {
    Rect rect;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, unsigned& a_foundCount, t_resultCallback a_resultCallback, void* a_context) {
    assert(a_node);
    assert(a_node->m_level >= 0);
    assert(a_rect);
